...
extern void vector_f_init_set(vector_f *v, size_t size, f_t init) {
    ...
    v->size = size;
    v->elements = (f_t *)malloc(sizeof(v->elements) * v->size);
...
extern void vector_i_init_set(vector_i *v, size_t size, i_t init) {
    ...
    size_t i;
    v->size = size;
    v->elements = (i_t *)malloc(sizeof(v->elements) * size);

    for ( i = 0; i < size; i++) v->elements[i] = init;
}

extern void vector_f_copy(vector_f *out, const vector_f *in) {
    size_t i;
    for (i = 0; i < out->size; i++) out->elements[i] = in->elements[i];
}
...
extern void vector_i_normalize(vector_i *v) {
    ...
    for (i = 0; i < v->size; i++) sum += v->elements[i];

    if ( sum != 0 ) {
        for (i = 0; i < v->size; i++) v->elements[i] /= sum;
    }
}
