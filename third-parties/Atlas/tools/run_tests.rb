...
def run_crash_test_once(test_name, target_name, test_dir, logfile, fp)
  ...
  test_out = test_dir + test_name + "." + target_name + ".crash.out"
  test_cmd = test_dir + test_name + \
             " `cat " + test_in + "`" + '>& ' + test_out
  ...
  system(cat_cmd)
  if (!status_run)
    msg = test_name + " failed crash testing"
    print_msg(msg, fp, logfile)
    return false
  else
    rm_cmd = "/bin/rm -f " + test_out
    system(rm_cmd)
    msg = test_name + " passed crash testing"
    print_msg(msg, fp, logfile)
    return true
  end
end
    ...
    if (!status_make)
      ...
      nvm_test_array.each do |test_name|
        ...
        if (status_run)
          status_recover = run_recovery(test_name, logfile, fp)
          if (!status_recover)
            msg = test_name + " failed recovery"
            print_msg(msg, fp, logfile)
            num_failed_tests += 1
          else
            msg = test_name + " passed recovery"
            print_msg(msg, fp, logfile)
            num_passed_tests += 1
          end
        else
