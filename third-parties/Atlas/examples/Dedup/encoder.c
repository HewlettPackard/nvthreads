...
static int GetNormalizedSize(int b_size)
{
  int i;
  static int z[] = {1024, 2*1024, 4*1024, 
		    8*1024, 16*1024, 32*1024, 64*1024};

  for (i = 0; i < (int)NELEM(z) - 1; i++) {
    if (b_size >= z[i] && b_size < z[i+1])
      return z[i];
  }

  EXIT_TRACE("block size %d is out of range(1K to 64K)\n",  b_size);
  return 0;
}
...
void *Compress(void * targs) {
  ...
  while(1) {
    ...
    if (compress_way == COMPRESS_GZIP) {
      ...
      /* compress the block */
      if (compress(pstr, &len, item->content, body->len) != Z_OK) {
        perror("compress");
...
ChunkProcess(void * targs) {
  ...
  while (1) {
    ...
    u_char * key = (u_char *)nvm_alloc(SHA1_LEN, hash_rgn_id);
    ...
    if ((entry = hashtable_search(cache, (void *)key)) == NULL) {
      ...
      struct pContent * value =
          (struct pContent *)nvm_alloc(sizeof(struct pContent), hash_rgn_id);
      value->len = 0;
      value->count = 1;
      value->content = NULL;
...
DataProcess(void * targs){
  ...
  if (avg_bsize == 0) {
    avg_bsize = GetNormalizedSize(conf->b_size);
    rf_win = 0;
    rabininit(rf_win);
  }
  ...
  //read from the file
  while ((srclen = read(fd, src, MAXBUF)) > 0) {
    tmp = 0; 
    p = src;
...
SendBlock(void * targs) 
{
  ...
  fd = open(conf->outfile, O_CREAT|O_WRONLY|O_TRUNC);
  if (fd < 0) {
    perror("SendBlock open");
    return NULL;
  }
  fchmod(fd, S_IRGRP | S_IRUSR | S_IROTH);
...
Encode(config * conf)
{
  ...
  */
  ...
  /* allocate a buffer for reading the source */
  if ((src = (byte *)malloc(MAXBUF)) == NULL)
    EXIT_TRACE("malloc src failed\n");
  ...
  for (i = 0; i < conf->nthreads; i ++) {
     ...
     pthread_create(&threads_anchor[i], NULL, FindAllAnchors, &anchor_thread_args[i].tid);
  }
 ...
 for (i = 0; i < conf->nthreads; i ++)
  ...
  FindAllAnchors(&generic_args);
  ChunkProcess(&generic_args);
  Compress(&generic_args);
