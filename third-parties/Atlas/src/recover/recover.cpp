...
int main(int argc, char **argv)
{
    ...
    if (!lsp)
    {
        fprintf(stderr, "Warning: No logs present\n");
        R_Finalize(argv[1]);
        exit(0);
    }
...
void R_Initialize(const char *s)
{
    ...
    {
        fprintf(stderr, "No log file exists, nothing to do ...\n");
        exit(0);
    }

    recovery_logs_id = NVM_FindRegion_priv(log_name, O_RDWR, true);
...
void R_Finalize(const char *s)
{
    ...
    fprintf(stderr, "Done bookkeeping\n");
}

LogStructure *GetLogStructureHeader()
{
    LogStructure **lsh_p =
        (LogStructure**)NVM_GetRegionRoot(recovery_logs_id);
    assert(lsh_p && "Region root is null: did you forget to set it?");
...
void Recover()
{
    ...
    for (Tid2Log::iterator ci = last_log_tracker.begin(); ci != ci_end; ++ ci)
        Recover(ci->first);
    fprintf(stderr, "Done reverting all data in the log ...\n");
}
