...
extern mcrs_err mcrs_f_init(matrix_crs_f *m, f_t empty, size_t col_sz, size_t row_sz) {
    ...
    m->values = (f_t *)pmalloc(sizeof(m->values) * col_sz);
    m->col_ind = (size_t *)pmalloc(sizeof(m->col_ind) * col_sz);
    m->row_ptr = (size_t *)pmalloc(sizeof(m->row_ptr) * row_sz);
...
//          	fprintf(stderr, "mcrs_f_set, before prealloc, m %p\n", m);
            	void* tmp = (void*)pmalloc(sizeof(m->row_ptr) * m->allocd_row);
		memcpy(m->row_ptr, tmp, sz_prev);
		pfree(m->row_ptr);
		m->row_ptr = (size_t*)tmp;//prealloc(m->row_ptr, sizeof(m->row_ptr) * m->allocd_row);
    ...
    if ( !found || newrow ) { // resize col_ind
		...
		void* tmp = (void*)pmalloc(sizeof(m->col_ind) * m->allocd_col);
		memcpy(m->col_ind, tmp, sz_prev);
		pfree(m->col_ind);
		m->col_ind = (size_t*)tmp;
		
		tmp = (void*)pmalloc(sizeof(m->values) * m->allocd_col);
