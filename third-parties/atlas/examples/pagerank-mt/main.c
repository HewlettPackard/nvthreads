...
int main(int argc, char **argv) {
    ...
    if ( argc != P_MIN ) {
        logd_e("Invalid arguments!\nUsage: pagerank <inputfile> <n_iterations> <n_threads> <n_lines> <n_rows> [<outputfile>], argc: %d\n", argc);
        return 1;
    }

#ifdef ATLAS
    NVM_Initialize();
    rgn_id = NVM_FindOrCreateRegion("pagerank-mt-atlas", O_RDWR, NULL);
    fprintf(stderr, "created nvm region with id: %d\n", rgn_id);
#endif
    ...
    size_t n_rows = atoi(argv[P_INITNR]);
    ...
    matrix_crs_f *adjm = (matrix_crs_f *)pmalloc(sizeof(matrix_crs_f));
    ...
    logd(LOGD_H, " sz_row=%zu sz_col=%zu n_col=%zu empty=%f\n",
    ...
    vector_f *init = (vector_f *)malloc(sizeof(vector_f));
