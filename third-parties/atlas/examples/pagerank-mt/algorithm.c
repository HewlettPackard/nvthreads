...
#define pmalloc(sz) nvm_alloc(sz, 0);
#define pfree(sz)
#endif
...
//  if ( data->thread_id == 0 ) {
//      fprintf(stderr, "setting dummy_char to %c (current: %c), start: %p, end: %p\n",
//              dummy_i+97, dummy_char[0], dummy_char, dummy_char + (1000*4096));
//      memset(dummy_char, dummy_i + 97, 4096 * 1000);
//      fprintf(stderr, "done setting dummy_char\n");
//      dummy_i++;
//  }
...
#ifdef ATLAS
			NVM_STR2(data->out->elements[i], data->out->elements[i], sizeof(data->out->elements[i]));
			NVM_STR2(data->loc->elements[i], 0.0, sizeof(data->loc->elements[i]));
			NVM_END_DURABLE();
#endif
	    }
...
extern mcrs_err mcrs_gmatrix_mult_vector_f_mt(logd_lvl_t lvl, vector_f *out, const matrix_crs_f *m, const vector_f *v, const size_t n_threads, const size_t n_iterations) {
    ...
    logd(lvl, "Initializing threads (n=%d)...\n", n_threads);
...
//  dummy_i = 0;
//  dummy_char = (char*)malloc(4096 * 1000);
    for (i = 0; i < n_threads; i++) {
        ...
        printf("allocating sz %zu for thread %zu / %zu\n", sizeof(vector_f) * sz, i, n_threads);
        data[i].loc = (vector_f *)pmalloc(sizeof(vector_f) * sz);
...
//      logd(lvl, "(");
        ...
        for (i = 0; i < n_threads; i++) {
//          fprintf(stderr, "Iteration %zu, creating thread %zu\n", n, i);
...
extern void gen_link_vector_crs(vector_i *linkv, int *empty, const matrix_crs_f *adjm) {
    ...
    for (i = 0; i < sz && i < adjm->sz_row; i++) {
        ...
        for (j = adjm->row_ptr[i]; j < ri_n; j++) linkv->elements[i] += adjm->values[j];
