...
static int FLAGS_num = 1000000; // 1M
...
class Benchmark {
 private:
  ...
  void Stop(const Slice& name) {
    ...
    double elapsed =  (finish - start_) * 1e6;
    fprintf(stdout, "%-12s : %11.3f micros/op;%s%s\n",
            name.ToString().c_str(),
            elapsed / done_,
            (message_.empty() ? "" : " "),
            message_.c_str());
    fprintf(stdout, "Time elapsed(us): %.5f", elapsed);
    if (FLAGS_histogram) {
 ...
 private:
    ...
    db_num_++;
    std::string test_dir;
    //if (FLAGS_db == NULL) //KUMUD
        Env::Default()->GetTestDirectory(&test_dir);
    //else  //KUMUD
    //   test_dir = FLAGS_db;  //KUMUD
...
extern "C" void NVM_PrintNumFlushes();
extern "C" void NVM_PrintStats();
