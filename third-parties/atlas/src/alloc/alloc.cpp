...
void *nvm_alloc_cache_line_aligned(size_t sz, uint32_t rid)
{
    ...
    if (next_cache_line + (intptr_t)sz >
        (intptr_t)region_data->base_addr + FILESIZE)
        assert(0);
    ...
    else
    {
        ...
        if ((intptr_t)region_data->curr_alloc_addr + (intptr_t)request_size +
            (intptr_t)NVM_get_metadata_size() >
            (intptr_t)region_data->base_addr + FILESIZE)
            assert(0);
        void *tmp = nvm_alloc(request_size, rid);

        if ((intptr_t)region_data->curr_alloc_addr + (intptr_t)sz + 
            (intptr_t)NVM_get_metadata_size() >
            (intptr_t)region_data->base_addr + FILESIZE)
            assert(0);
        void *ret = nvm_alloc(sz, rid);
