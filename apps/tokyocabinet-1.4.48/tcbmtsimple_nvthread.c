...
//#endif
  ...
  /* check input */
  if (argc < 6) {
    fprintf(stderr, "Usage: %s database nthreads nqueries recordsize writepercent\n", argv[0]);
    exit(-1);
  }
  ...
  printf("%s %d threads, %d queries, %d bytes/record, %d queries/thread, %d writepercent\n",
         argv[0], NTHREADS, NQUERIES, RECBUFSIZE, NQUERIES_PER_THREAD, WRITE_PERCENT);
