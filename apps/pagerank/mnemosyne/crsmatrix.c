...
extern mcrs_err mcrs_i_init(matrix_crs_i *m, i_t empty) {
    ...
    m->values = pmalloc(sizeof(m->values));
    m->col_ind = pmalloc(sizeof(m->col_ind));
    m->row_ptr = pmalloc(sizeof(m->row_ptr));
...
extern mcrs_err mcrs_f_init(matrix_crs_f *m, f_t empty, size_t init_val_sz) {
    ...
    m->values = pmalloc(sizeof(m->values) * init_val_sz);
    m->col_ind = pmalloc(sizeof(m->col_ind) * init_val_sz);
    m->row_ptr = pmalloc(sizeof m->row_ptr);
...
extern mcrs_err mcrs_i_set(matrix_crs_i *m, size_t x, size_t y, i_t val, mcrs_set_mode md) {
    ...
    if ( y >= m->sz_row ) { // resize row_ptr
        if ( y >= m->allocd_row ) {
            ...
            m->row_ptr = prealloc(m->row_ptr, sizeof(m->row_ptr) * m->allocd_row);
        }
