...
// Check all versions to see if the target of the provided le is present.
// If present, nullify the acquire node's pointer.
// TODO: don't we need to do something with ReleaseInfo hash table here?
void NullifyAcqToDeletedRel(LogEntry *le, const LogVersions & log_v)
{
    // Starting in reverse order may produce a match faster, just a heuristic
    int i = log_v.size() - 1;
    for (; i >= 0; --i)
    {
        const Log2Bool & rel_log = log_v[i].Rel_;
        Log2Bool::const_iterator rel_ci =
            rel_log.find((LogEntry *)(le->ValueOrPtr_));
        if (rel_ci != rel_log.end())
        {
            le->ValueOrPtr_ = 0;
#if !defined(_DISABLE_LOG_FLUSH) && !defined(DISABLE_FLUSHES)
            NVM_FLUSH(&le->ValueOrPtr_);
#endif            
            return;
        }
    }
                        ...
                        const LogVersions & log_v)
{
    ...
    for (PendingList::iterator ci = pl->begin(); ci != ci_end; ++ ci)
    {
        ...
        {
//            NullifyAcqToDeletedRel(le, log_v);
                    ...
                    const Log2Bool & acq_map, const OcsMap & ocs_map)
{
    ...
    LogStructure *lsp = (LogStructure *) ALAR(*log_structure_header_p);
    assert(lsp);
...
void CreateMapFromOwnerTab(Log2Bool * log_map)
{
    int i;
    for (i = 0; i < TAB_SIZE; ++i)
    {
        OwnerInfo * owner = (OwnerInfo *) ALAR(OwnerTab[i]);
        while (owner)
        {
            log_map->insert(make_pair(owner->LogAddr_, true));
            owner = owner->Next_;
        }
    }
}
...
void DestroyLogs(LogVersions *log_v)
{
    ...
    // Create a hash table of log entries found in OwnerTab. Even if new
    // entries are added between now and query, they don't matter since
    // they can't be in the already-deleted TCSes.
    Log2Bool log_map;
    CreateMapFromOwnerTab(&log_map);
    ...
    {
        LogStructure *gh = (LogStructure*) ALAR(*log_structure_header_p);
        assert(gh);
        ...
        while (tmp_gh)
        {
            ...
            do
            {
                ...
                if (log_map.size() &&
                    hasReferenceFromOwnerTab(curr_le, log_map))
                {
                    done = true;
                    break;
                }
    ...
    !defined(_DISABLE_DATA_FLUSH)
            ...
            if (logs_ci == log_v->begin())
            {
                UtilVerboseTrace2(
                    helper_trace_file,
                    "[HELPER] Destroying a single header list\n");
    ...
    UtilVerboseTrace2(helper_trace_file,
                      "[HELPER] Destroying headers within a loop\n");
...
void DestroyLS(LogStructure * lsp)
{
    ...
    UtilTrace2(helper_trace_file, "[HELPER] Destroying log header ");
    ...
    while (lsp)
    {
        ...
        nvm_free(del_lsp, true /* do not log */);
    }
...
void DestroyLogEntries(const LogEntryVec & le_vec)
{
    ...
    UtilTrace2(helper_trace_file, "[HELPER] Destroying log entries ");
    ...
    for (LogEntryVec::const_iterator ci = le_vec.begin(); ci != ci_end; ++ ci)
    {
        ...
        if (isRelease(*ci)) AddEntryToDeletedMap(*ci, (*ci)->Size_);
...
void FixupNewEntries(LogStructure **cand, const LSVec & new_entries)
{
    ...
    UtilVerboseTrace2(
        helper_trace_file, "[HELPER] Fixup: created header nodes: ");
...
// TODO we need to match the generation number as well.
bool hasReferenceFromOwnerTab(LogEntry *le, const Log2Bool & log_map)
{
    // only a release log entry can be found in OwnerTab
    if (isRelease(le) && log_map.find(le) != log_map.end())
    {
        UtilTrace3(helper_trace_file,
                  "Bailing out: reference %p found in owner-table\n", le);
