...
void * nvm_alloc(size_t sz, uint32_t rid)
{
    ...
    {
        fprintf(stderr, "[ALLOC] Invalid region %d\n", rid);
        PrintBackTrace();
...
void nvm_free(void * p)
{
    ...
    {
        fprintf(stderr, "[ALLOC] assert: %p %ld %ld\n",
                (size_t *)p, *((size_t *)p),
...
{
    return NVM_CreateRegion_priv(name, flags);
}
                                ...
                                bool is_lock_held, void *forced_base_addr)
{
    ...
    *root_ptr = 0;
    ...
    AtlasTrace(stderr, "[ALLOC] CreateRegion created region name=%s id %d\n",
               name, num_entries);
...
uint32_t NVM_CreateLogFile(const char *name)
{
    ...
    *root_ptr = 0;
    ...
    AtlasTrace(stderr, "[ALLOC] Log file %s created with id %d\n",
                                     ...
                                     bool is_forced_map, int *is_created_p)
{
    ...
    if (region_entry)
    {
        ...
        AtlasTrace(stderr,
                   "[ALLOC] NVM_FindOrCreateRegion found region id %d\n",
                   region_entry->id);
        RegionDump(region_entry);
...
{
    return NVM_FindRegion_priv(name, flags);
}

// TODO this is called from recover phase, so that is creating 2 different
// mappers!
uint32_t NVM_FindRegion_priv(const char *name, int flags, bool is_forced_map)
{
    assert(strlen(name) <= MAXLEN);
    ...
    const char * name, int flags, intptr_t * base_addr)
{
    ...
    fprintf(stderr, "[ALLOC] Map existing file %s at base_address %p\n",
            s, base_addr);
