#include <stdlib.h>
#include <stdio.h>
#include <unistd.h>
#include <pthread.h>
#include "nvrecovery.h"

pthread_mutex_t glock1;
pthread_mutex_t glock2;
pthread_cond_t cv;
int A;
int *B;
#define LIMIT_A 100
void* thread_a(void *args) {
    pthread_mutex_lock(&glock1);
    sleep(1);
    printf("thread_a locked g1\n");
    pthread_mutex_lock(&glock2);
    printf("thread_a locked g2\n");
    *B = 99;
    sleep(1);
    printf("thread_a waiting...\n");
    while (A != 1) {
        sleep(1);
        pthread_cond_wait(&cv, &glock2);
        printf("A = %d\n", A);
    }
    printf("thread_a is up!\n");
    pthread_mutex_unlock(&glock2);
    printf("thread_a unlocked g2\n");
    sleep(1);
    pthread_mutex_unlock(&glock1);
    printf("thread_a unlocked g1\n");
    return 0;
}
void* thread_b(void *args) {
    sleep(2);
    pthread_mutex_lock(&glock2);
    printf("thread_b locked g2\n");
    sleep(5);
    A = 1;
    pthread_cond_signal(&cv);
    *B = *B + A;
    printf("A = %d, B = %d\n", A, *B);
    pthread_mutex_unlock(&glock2);
    printf("thread_b unlocked g2\n");
    return 0;
}
int main(void) {
    pthread_t tid1, tid2;
    A = 0;
    B = (int *)malloc(sizeof(int));
    *B = 0;
    printf("glock1: %p, glock2: %p, cond: %p\n", &glock1, &glock2, &cv);
    pthread_mutex_init(&glock1, NULL);
    pthread_mutex_init(&glock2, NULL);
    pthread_cond_init(&cv, NULL);
    pthread_create(&tid1, NULL, thread_a, NULL);
    pthread_create(&tid2, NULL, thread_b, NULL);
    pthread_join(tid2, NULL);
    pthread_join(tid1, NULL);

    printf("final A = %d, B= %d\n", A, *B);
    return 0;
}

